"""
    A simple test to check if everything is allright with the Encoder block implementation
"""
...
encoder_block = EncoderBlock(d_model=embedding_dim, h=h, d_k=d_k, d_v=d_v, d_ff=d_ff)

number_of_samples = 100

embeddings = torch.rand(number_of_samples, embedding_dim)

print("encoder_block.forward(embeddings):")
print(encoder_block.forward(embeddings))
print("encoder_block.forward(embeddings).shape:")
print(encoder_block.forward(embeddings).shape)
