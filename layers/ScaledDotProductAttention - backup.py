    ...
    def forward(self, embeddings):
        ...
        if (self.masking == False):
            if (self.is_matrix(K)):
                ...
                print(K.shape)
                print("self.SoftmaxForMatrices(torch.div(torch.matmul(Q, torch.transpose(K, -2, -1)), math.sqrt(self.d_k))).shape: (ScaledDotProductAttention)")
                print(self.SoftmaxForMatrices(torch.div(torch.matmul(Q, torch.transpose(K, -2, -1)), math.sqrt(self.d_k))).shape)
                print("torch.matmul(self.SoftmaxForMatrices(torch.div(torch.matmul(Q, torch.transpose(K, -2, -1)), math.sqrt(self.d_k))), V).shape: (ScaledDotProductAttention)")
                print(torch.matmul(self.SoftmaxForMatrices(torch.div(torch.matmul(Q, torch.transpose(K, -2, -1)), math.sqrt(self.d_k))), V).shape)
                """
                ...
                print("K.shape: (ScaledDotProductAttention)")
                print(K.shape)
                print("self.SoftmaxForVectors(torch.div(torch.matmul(Q, K), math.sqrt(self.d_k))).shape: (ScaledDotProductAttention)")
                print(self.SoftmaxForVectors(torch.div(torch.matmul(Q, K), math.sqrt(self.d_k))).shape)
                print("torch.mul(self.SoftmaxForVectors(torch.div(torch.matmul(Q, K), math.sqrt(self.d_k))), V).shape: (ScaledDotProductAttention)")
                print(torch.mul(self.SoftmaxForVectors(torch.div(torch.matmul(Q, K), math.sqrt(self.d_k))), V).shape)
                """
            ...
            print("K.shape: (ScaledDotProductAttention)")
            print(K.shape)
            print("self.SoftmaxForMatrices(torch.div(torch.add(torch.matmul(Q, torch.transpose(K, -2, -1)), M), math.sqrt(self.d_k))).shape (ScaledDotProductAttention)")
            print(self.SoftmaxForMatrices(torch.div(torch.add(torch.matmul(Q, torch.transpose(K, -2, -1)), M), math.sqrt(self.d_k))).shape)
            print("torch.matmul(self.SoftmaxForMatrices(torch.div(torch.add(torch.matmul(Q, torch.transpose(K, -2, -1)), M), math.sqrt(self.d_k))), V).shape: (ScaledDotProductAttention)")
